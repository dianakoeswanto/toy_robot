...
describe PlaceCommand do
  ...
  context '#execute' do
    it 'should place robot on the table top' do
      ...
      PlaceCommand.from_string('PLACE 0,0,NORTH').execute(robot)
    end
  end

  context 'from_string' do
    it 'should return an instance of PlaceCommand' do
      command = PlaceCommand.from_string('PLACE 0,0,NORTH')
    ...
    it 'should throw argument exception when command string does not begin with PLACE' do
      expect { PlaceCommand.from_string('INVALID_COMMAND 0,0,NORTH') }.to raise_error(ArgumentError)
    end

    it 'should throw argument exception when first argument is not a valid integer' do
      expect { PlaceCommand.from_string('PLACE 1.3,0,NORTH') }.to raise_error(ArgumentError)
    end

    it 'should throw argument exception when second argument is not a valid integer' do
      expect { PlaceCommand.from_string('PLACE 1, 0.5, NORTH') }.to raise_error(ArgumentError)
    end

    it 'should throw argument exception when given an invalid orientation' do
      expect { PlaceCommand.from_string('PLACE 0,0,INVALID_ORIENTATION') }.to raise_error(ArgumentError)
    end
  end
end
